...
- `title`：開場標題卡（可選 `props.src` 滿版背景圖：自動暗化壓字＋緩慢推進；無圖時為動態漸層＋漂浮光點）
- `image`：情境插圖 + 一句說明（**滿版 cover + Ken Burns** 慢速縮放位移，方向依圖片雜湊自動錯開）
- `stepCards`：2～4 個步驟/重點卡片依序彈跳滑入（鮮豔漸層背景＋漂浮圓點）
...
- [x] 滿版動態版型：image 場景 Ken Burns、title/summary 可選滿版背景圖、stepCards 動態背景（2026-07-13，Atlas）
